...
public class CartController {
    ...
    public Result addGoodsToCartList(Long itemId, Integer num, HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            // 将请求的数据封装到cart中
            Cart cart = new Cart();
            Item item = cartService.findOne(itemId);//用库存id  类查商家id
            cart.setSellerId(item.getSellerId());   // 存储的商家id
            List<OrderItem> orderItemList = new ArrayList<>();
            OrderItem orderItem = new OrderItem();
            orderItem.setItemId(itemId);    // 库存id  因为搜身 我后台只需要 id  就能查出来
...
//            cartList.contains() // 没问题，但是这个方法找不到具体的位置
            int sellerIndexOf = cartList.indexOf(cart);// 说明：判断商品是否属于同一个商家 //想着 超市存包
            if(sellerIndexOf != -1){
                // 同一个商家：继续判断，判断是否属于同款商品
                Cart oldCart = cartList.get(sellerIndexOf);//这和是原有的  怎样找到 就是传个值
