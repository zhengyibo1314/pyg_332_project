...
public class CartServiceImpl implements CartService {
    ...
    public List<Cart> autoDataToCart(List<Cart> cartList) {
        for (Cart cart : cartList) {
            Seller seller = sellerDao.selectByPrimaryKey(cart.getSellerId()); //商家id
            cart.setSellerName(seller.getNickName());   // 商家店铺名称
            List<OrderItem> orderItemList = cart.getOrderItemList();
            // 填充购物项中的数据
            for (OrderItem orderItem : orderItemList) {
                Item item = itemDao.selectByPrimaryKey(orderItem.getItemId()); //库存id
...
//这个是讲我们购物车保存到redis 中
    ...
    public void saveCartToRedis(String username, List<Cart> newCartList) {
     //如果redis 中有叫做老车     // 取出老车
        ...
        // 将老车保存到redis中  为什么保存老车因为老车有可能多 包含新车
